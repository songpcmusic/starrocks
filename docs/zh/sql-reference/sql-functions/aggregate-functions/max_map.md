...
description: "按 key 独立计算多个 MAP 值中的最大值。"
---
...
按 key 独立计算多行 MAP 中非 NULL value 的最大值。某行中缺失的 key 不参与计算，也不会贡献默认值。
...
CHAR、VARCHAR、DATE 和 DATETIME。

value 支持 BOOLEAN、各整数类型、FLOAT、DOUBLE、DECIMALV2、DECIMAL32/64/128、CHAR、VARCHAR、DATE 和
DATETIME。
...
对于浮点 value，任何非 NaN 都大于 NaN。因此 NaN 与普通数混合时返回最大的非 NaN 值；某个 key 的
...
- `NULL` value 不参与最大值计算，但保留对应 key；如果某个 key 的所有 value 都为 `NULL`，结果中的
  value 也是 `NULL`。
