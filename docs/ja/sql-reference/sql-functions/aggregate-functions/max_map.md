...
description: "複数の MAP 値についてキーごとの最大値を計算します。"
---
...
複数行の MAP に対して、キーごとに非 NULL value の最大値を計算します。ある行に存在しないキーは計算に
...
DECIMAL32/64/128、CHAR、VARCHAR、DATE、DATETIME をサポートします。DECIMAL256 key は未対応です。

value は BOOLEAN、各整数型、FLOAT、DOUBLE、DECIMALV2、DECIMAL32/64/128/256、CHAR、VARCHAR、DATE、
DATETIME をサポートします。
...
浮動小数点 value では、すべての非 NaN 値を NaN より大きいものとして扱います。そのため NaN と非 NaN が
混在する場合は最大の非 NaN 値を返し、すべて NaN のキーは NaN を返します。Infinity は通常の数値順序に
従います。
...
- `NULL` value は最大値の計算には含めませんが、key は保持します。ある key の value がすべて `NULL`
