...
public:
    ...
    void convert_to_serialize_format(FunctionContext* ctx, const Columns& src, size_t chunk_size,
        ...
        const auto& src_offsets = src_map_column->offsets().get_data();
        size_t entry_begin = src_offsets[0];
        size_t entry_count = src_offsets[chunk_size] - entry_begin;

        dst_map_column->keys_column()->append(*src_map_column->keys_column(), entry_begin, entry_count);
        dst_map_column->values_column()->append(*src_map_column->values_column(), entry_begin, entry_count);
...
private:
    ...
    static void append_key(const HashKeyType& key, Column* output) {
        ...
        auto* data = down_cast<KeyColumnType*>(nullable->mutable_data_column());
        if constexpr (lt_is_string<KT>) {
    ...
    static void append_value(const ValueState& state, Column* output) {
        ...
        auto* nullable = down_cast<NullableColumn*>(output);
        auto* data = down_cast<ValueColumnType*>(nullable->mutable_data_column());
    ...
    static void append_state(const State& state, Column* output) {
        ...
        Column* key_column = map_column->keys_column().get();
        Column* value_column = map_column->values_column().get();
        size_t appended = 0;
        ...
        offsets->append(offsets->get_data().back() + appended);
