...
struct SumMapAggregateFunctionState : public AggregateFunctionEmptyState {
    ...
    typename ValueResultColumnType::MutablePtr value_result_column = ValueResultColumnType::create_mutable();
...
private:
    ...
    void append_key(Column* key_column, const KeyValue& key) const {
        ...
        auto* key_data_col = down_cast<KeyColumnType*>(nullable_key->mutable_data_column());
    ...
    void append_value_data(Column* value_data_col, ValueResultCppType value) const {
        ...
        auto* data_column = down_cast<ValueResultColumnType*>(nullable_value_col->mutable_data_column());
    ...
    void finalize_map_column(Column* to, MapColumn* map_column, size_t total_entries) const {
        // Update map offsets
        auto* offsets_col = map_column->offsets_column().get();
        offsets_col->append(offsets_col->get_data().back() + total_entries);
    }
...
public:
    ...
    void convert_to_serialize_format(FunctionContext* ctx, const Columns& src, size_t chunk_size,
        ...
        const auto* src_map_column = down_cast<const MapColumn*>(ColumnHelper::get_data_column(src[0].get()));
        ...
        // append keys
        dst_map_column->keys_column()->append(*src_map_column->keys_column(), 0, element_size);

        DCHECK(dst_map_column->values_column()->is_nullable());

        NullableColumn* nullable_dst_values_column = down_cast<NullableColumn*>(dst_map_column->values_column().get());
        auto& null_data = nullable_dst_values_column->null_column_data();
        ValueResultColumnType* dst_values_column =
                down_cast<ValueResultColumnType*>(nullable_dst_values_column->mutable_data_column());
        ...
        auto& dst_offsets = dst_map_column->offsets_column()->get_data();
