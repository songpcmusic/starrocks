...
private:
    ...
    static void register_functions(AggregateFuncResolver* resolver) {
        ...
        auto min_function = std::make_shared<MinMaxMapAggregateFunction<kt, vt, HashMap>>(true);
        auto max_function = std::make_shared<MinMaxMapAggregateFunction<kt, vt, HashMap>>(false);
        resolver->add_aggregate_mapping<kt, vt, State>("min_map", false, min_function);
        resolver->add_aggregate_mapping<kt, vt, State>("max_map", false, max_function);
    }
};
