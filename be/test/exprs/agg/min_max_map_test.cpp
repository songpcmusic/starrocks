...
protected:
    ...
    static const AggregateFunction* resolve(const std::string& function_name, const TypeDescriptor& map) {
        return get_aggregate_function(function_name, map.children[0].type, map.children[1].type, false,
                                      TFunctionBinaryType::BUILTIN);
    }

    static std::unique_ptr<FunctionContext> test_context(const TypeDescriptor& map) {
        std::vector<FunctionContext::TypeDesc> argument_types = {AnyValUtil::column_type_to_type_desc(map)};
        return std::unique_ptr<FunctionContext>(FunctionContext::create_test_context(
                std::move(argument_types), AnyValUtil::column_type_to_type_desc(map)));
    }

    static std::unique_ptr<FunctionContext> context_with_pool(MemPool* mem_pool, const TypeDescriptor& map) {
        std::vector<FunctionContext::TypeDesc> argument_types = {AnyValUtil::column_type_to_type_desc(map)};
        return std::unique_ptr<FunctionContext>(FunctionContext::create_context(
                nullptr, mem_pool, AnyValUtil::column_type_to_type_desc(map), argument_types));
    }
    ...
    static MapColumn::Ptr string_map(const std::vector<std::optional<std::string>>& keys,
    ...
    static MapColumn::Ptr int_double_map(const std::vector<int32_t>& keys,
...
TEST_F(MinMaxMapTest, updateAndFinalizeIntegerMaps) {
    ...
    std::unique_ptr<FunctionContext> context = test_context(map);
    ...
    for (const auto& test_case : {std::pair{"min_map", std::vector<std::optional<int32_t>>{4, 0, 20, std::nullopt}},
        ...
        const AggregateFunction* function = resolve(test_case.first, map);
        ...
        ColumnPtr result = ColumnHelper::create_column(map, false);
...
TEST_F(MinMaxMapTest, partialStatesMergeAcrossBothSerializationPaths) {
    ...
    for (const auto& test_case : {std::pair{"min_map", std::vector<std::optional<int32_t>>{3, 100}},
        ...
        ColumnPtr serialized = ColumnHelper::create_column(map, false);
        ...
        ColumnPtr merged_result = ColumnHelper::create_column(map, false);
        ...
        ColumnPtr streaming_input_column = streaming_input;
        Columns source{streaming_input_column};
        ColumnPtr streaming_serialized = ColumnHelper::create_column(map, false);
        function->convert_to_serialize_format(context.get(), source, streaming_input_column->size(),
                                              &streaming_serialized);
        ...
        ColumnPtr streaming_result = ColumnHelper::create_column(map, false);
...
TEST_F(MinMaxMapTest, booleanSignatureIsRegisteredForUntypedNull) {
    ...
    for (const char* function_name : {"min_map", "max_map"}) {
        const AggregateFunction* function = resolve(function_name, map);
...
TEST_F(MinMaxMapTest, allSupportedKeyAndValueTypesAreRegistered) {
    ...
    for (const char* function_name : {"min_map", "max_map"}) {
        for (LogicalType key_type : supported_key_types) {
            ...
            EXPECT_NE(nullptr, resolve(function_name, map)) << function_name << " key type " << key_type;
        }
        for (LogicalType value_type : supported_value_types) {
            ...
            EXPECT_NE(nullptr, resolve(function_name, map)) << function_name << " value type " << value_type;
        }
    }
}
...
TEST_F(MinMaxMapTest, decimalV2ValuesPreserveInputType) {
    {
...
TEST_F(MinMaxMapTest, stringKeysAndSelectedValuesOutliveInput) {
    ...
    MemPool mem_pool;
    std::unique_ptr<FunctionContext> context = context_with_pool(&mem_pool, map);
