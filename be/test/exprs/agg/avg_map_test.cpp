...
protected:
    ...
    static MapColumn::MutablePtr string_int_map(const std::vector<std::optional<std::string>>& keys,
                                                const std::vector<std::optional<int32_t>>& values) {
        DCHECK_EQ(keys.size(), values.size());
    ...
    static const AggregateFunction* resolve(const TypeDescriptor& argument_type, const TypeDescriptor& return_type) {
        return get_aggregate_function("avg_map", return_type, {argument_type}, false, TFunctionBinaryType::BUILTIN);
    }
...
TEST_F(AvgMapTest, updatePreservesMissingDuplicateAndNullEntries) {
    ...
    std::unique_ptr<FunctionContext> context(FunctionContext::create_test_context({argument_type}, return_type));
    ...
    MutableColumnPtr result = ColumnHelper::create_column(return_type, false);
...
TEST_F(AvgMapTest, mergeUsesWeightedPartialState) {
    ...
    MutableColumnPtr serialized = BinaryColumn::create();
...
TEST_F(AvgMapTest, convertToSerializeFormatRetainsPerKeyCounts) {
    ...
    Columns source{input};
    MutableColumnPtr serialized = BinaryColumn::create();
    function->convert_to_serialize_format(context.get(), source, input->size(), serialized);
...
TEST_F(AvgMapTest, convertToSerializeFormatPreservesVersionOneWireLayout) {
    ...
    std::unique_ptr<FunctionContext> int_context(
            FunctionContext::create_test_context({int_argument_type}, int_return_type));
    ...
    auto int_input = int_map({1, std::nullopt, 2, 3}, {10, 20, std::nullopt, 30}, {3, 3, 4});
    Columns int_source{int_input};
    MutableColumnPtr int_serialized = BinaryColumn::create();
    ...
    std::unique_ptr<FunctionContext> string_context(
            FunctionContext::create_test_context({string_argument_type}, string_return_type));
    const AggregateFunction* string_function = resolve(string_argument_type, string_return_type);
    ASSERT_NE(nullptr, string_function);

    auto string_input = string_int_map({"ab", std::nullopt}, {4, 6});
    Columns string_source{string_input};
    MutableColumnPtr string_serialized = BinaryColumn::create();
...
TEST_F(AvgMapTest, stringKeysAndValuesOutliveInput) {
    ...
    std::unique_ptr<FunctionContext> context(
            FunctionContext::create_context(nullptr, &mem_pool, return_type, {argument_type}));
    const AggregateFunction* function = resolve(argument_type, return_type);
    ASSERT_NE(nullptr, function);
...
TEST_F(AvgMapTest, decimalValuesUseWideAccumulatorAndKeepScaleUntilFinalize) {
    ...
    auto input = MapColumn::create(NullableColumn::create(std::move(key_data), NullColumn::create(kEntryCount, 0)),
                                   ...
                                   UInt32Column::create());
    input->offsets_column_raw_ptr()->append(kEntryCount);
